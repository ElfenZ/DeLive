...
| AI 概要 | AI 校正 | AI チャット |
|:---:|:---:|:---:|
| 要約、アクション項目、キーワード、チャプター | クイック修正 / 確認して修正、差分表示 | 引用付きのマルチスレッド会話 |
...
> 📖 構成の詳細：[概要](https://docs.delive.me/architecture/overview) · [プロバイダー](https://docs.delive.me/architecture/providers) · [Electron IPC](https://docs.delive.me/architecture/electron-ipc) · [データフロー](https://docs.delive.me/architecture/data) · [セキュリティ](https://docs.delive.me/architecture/security)

## 📁 プロジェクト構成

```text
DeLive/
├── electron/          # Electron メインプロセス、ウィンドウ、トレイ、IPC、アップデート、ランタイム、Open API サーバー
├── frontend/          # React レンダラー、プロバイダー、Store、UI コンポーネント、テスト
├── shared/            # 共有 TypeScript 契約とプロキシヘルパー
├── server/            # デバッグ用スタンドアロンプロキシサーバー
├── mcp/               # AI エージェント向け MCP サーバー（Claude、Cursor など）
├── skills/            # エージェントスキル定義
├── scripts/           # アイコン生成、ランタイム準備、リリースノート
├── docs/              # VitePress ドキュメントサイトのソース
├── landing/           # ランディングページのソース
└── package.json
```
...
DeLive は、ローカル REST API、リアルタイム WebSocket、MCP サーバーを通じて文字起こしデータを外部ツールから利用できます。これらはデフォルトで無効で、必要に応じて Bearer Token 認証を設定できます。
...
- **プロバイダープロキシ**は Electron に組み込まれているため、通常利用では別途バックエンドを起動する必要はありません。
...
- [BiBi-Keyboard](https://github.com/BryceWG/BiBi-Keyboard) — マルチプロバイダー構成の着想
- [ByteDance](https://www.bytedance.com) — Volcengine 音声認識サービスと Lark AI Campus Challenge のサポート
- [LINUX.DO](https://linux.do) コミュニティ — 多くの学びと温かいサポートをいただきました
