...
会议上下文是供转写与 AI 纠错复用的不含凭据输入：

| 字段 | 行为 |
|------|------|
| **会议背景** | 会议主题、领域、产品和技术事实，最多 4,000 个 Unicode code point |
| **纠错指导** | 仅供 AI 使用的纠错偏好，最多 2,000 个 Unicode code point |
| **用于 AI 纠错** | 默认开启；以有总预算的 JSON 参考数据注入，不改变固定 Patch 协议 |
| **发送到 Soniox** | 默认关闭；开启后将背景和已启用术语的目标词作为 Soniox context 发送 |
| **术语表** | 支持已知 `source -> target` 映射和仅填写 target 的候选正确术语 |

录制页和文件转写页提供单次上下文编辑器。每个任务可继承全局上下文、单次覆盖或清空；有效上下文在任务开始时冻结，之后修改设置不会影响重连或自动纠错。
...
Electron 优先使用端口 `23456`，端口冲突时会依次尝试 `23457–23460`。此处显示的 URL 是运行时事实来源；发生回退时，页面会提示外部 MCP 或脚本手动更新 `DELIVE_API_URL`。
