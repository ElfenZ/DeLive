...
3. 通过 `DELIVE_API_URL` 配置的地址调用 DeLive **REST API** 获取数据
...
`23456` 是 Electron 首选端口。若 DeLive 回退到 `23457–23460`，请从 **设置 > Open API** 复制实际 REST 地址，并更新 `DELIVE_API_URL`。外部 MCP 进程不会自动发现回退端口。
...
获取完整会话详情，包含转录、纠错后文本（如有）、AI 摘要、思维导图和问答历史。纠错元数据仅为摘要，不暴露 Patch 或草稿明细。
